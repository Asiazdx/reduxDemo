...
if(process.argv.indexOf('-p')>-1){//生产环境
	...
	publicPath='/myReact/dist/';
	path=_dirname+'/myReact/dist/';
}
...
module.exports={
	...
	module:{
		loaders:[
			...
			{
				text:/\.(eto|woff|svg|ttf|woff2|gif|appcache)(\?|$)/,
				exclude:/^node_modules$/,
			...
			},
			{
				test:/\.(png|jpg)$/,
				exclude:/^node_modules$/,
				loader:'url?limit=20000&name=[name].[ext]'//注意后面那个limit的参数，当你的图片大小小于这个限制的时候，会自动启用base64编码图片
			},
			{
				test:/\.jsx$/,
				exclude:/^node_modules$/,,
