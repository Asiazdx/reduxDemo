...
*@extends{Component}
*/
class Nav extends from Component{
	render(){
		var setCur={};
			...
			<nav className="index-nav">
				<ul data-flex="box:mean">
					...
					<li className={setCur.good}>
						<Link to="/?tab=good" activeClassName="active">推荐</Link>
					</li>
...
class ListItem extends Component{
	render(){
		let {id,title,author,visit_count,replay_count,create_at,last_reply_at}=this.props;
		return(
			<li>
				<Link to={`/topic/${id}`}>
					...
					<div className="bottom" data-flex="box:first">
						...
						<div className="con" data-flex="dir:top main:center">
							<p data-flex="cross:center box:last">
								...
								<span className="count">{replay_count}/{visit_count}</span>
							</p>
							<p data-flex="cross:center box:last">
								<time className="create">{Tool.formatDate(create_at)}</tiem>
